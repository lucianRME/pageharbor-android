...
Consequences:
This approves a Scan Result action that invokes the UI-independent, caller-driven coordinator to run or consume local OCR, prepare a private-cache searchable PDF, and copy it to a user-selected SAF URI. It does not change the existing scanner-PDF save, share, or JPEG-export flows. The coordinator must delete prepared PDFs after write success, write failure, cancellation, or explicit destination-selection cancellation. PdfBox-Android is necessary because Android `PdfDocument` lacks verified invisible-text rendering and embedded Unicode-font control. Do not declare Bouncy Castle directly unless a later PdfBox compatibility requirement proves necessary. Record the release-size measurement with the implementation. The existing scanner PDF remains the fallback until page-rotation and JPEG-fidelity validation plus multiple-viewer search/selection/copy validation pass. All OCR geometry and prepared files remain active-session local data and must never be logged, retained as a library, or transmitted.
